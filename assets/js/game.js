...
var fight = function (enemy) {
    ...
    while (playerInfo.health > 0 && enemy.health > 0) {
        if (isPlayerTurn) {
            // ask if they'd like to fight or skip using fightOrSkip fucntion
            if (fightOrSkip()) {
                break;
            }
...
for (var i = 0; i < enemyInfo.length; i++) {
    ...
    // after loo[ ends, we are either out of playerhealth or enemies to fight, so run the endGame function
    endGame();
};
...
var endGame = function() {
    ...
    // check localStore for high score, if it's not there, use 0
